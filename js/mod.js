...
let modInfo = {
	name: '进化增量',
	nameEN: 'Evolution Incremental', // When you open the otherLanguageMod, this is the second language
	id: 'evo-inc',
	author: 'AntiDim19728',
	pointsName: 'points',
...
var displayThings = [
	function () {
		if (options.ch == undefined && modInfo.otherLanguageMod == true) {
			return '<big><br>You should choose your language first<br>你需要先选择语言</big>';
		}
		return (
			'<div class="res">' +
			displayThingsRes() +
			'</div><br><div class="vl2"></div></span>'
		);
	},
];
...
function getPointsDisplay() {
	...
	if (player.devSpeed && player.devSpeed != 1) {
		a += options.ch
			? '<br>时间加速: ' + format(player.devSpeed) + 'x'
			: '<br>Dev Speed: ' + format(player.devSpeed) + 'x';
	}
	if (player.offTime !== undefined) {
		a += options.ch
			? '<br>离线加速剩余时间: ' + formatTime(player.offTime.remain)
			: '<br>Offline Time: ' + formatTime(player.offTime.remain);
	}
	a += '<br>';
	if (!(options.ch == undefined && modInfo.otherLanguageMod == true)) {
		a += `<span class="overlayThing">${
			options.ch || modInfo.languageMod == false ? '你有' : 'You have'
		} <h2  class="overlayThing" id="points"> ${format(player.points)}</h2> ${
			modInfo.pointsName
		}</span>`;
		if (canGenPoints()) {
			a +=
				`<br><span class="overlayThing">(` +
